...
| **Nothing drives the desktop application.** `apps/desktop` bundles the same `dist` the browser install ships, and the suite and the probes cover that build in Chromium. The bundle itself — launched, drawing, reading a table through a worker on Tauri's own scheme — has only been looked at by hand.                                                           | A webview-only failure (a worker or a download the custom scheme refuses) would reach a release. `tauri-driver` could drive it the way `install-check` drives the installed page. |
