...
| Suite                | Cases | Proves                                                                                                                                                                                                |
| -------------------- | ----: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `core/test`          |   334 | Commands against worlds, the history graph and branching, session state, entities, ids, placement, query chains, bindings, chart specifications                                                       |
| `table/test`         |   137 | The block cache and its eviction, row windows, smooth scroll and velocity, prefetch, chunks and vectors, column summaries, formatting                                                                 |
| `chart/test`         |    96 | The reduction — grouping, cross-tabulation, limits, gaps versus zeroes, sampled versus exact — and mark picking                                                                                       |
| `chart-echarts/test` |   146 | ECharts driven headless into geometry: polygons, text, triangulation, layout bounds                                                                                                                   |
| `exasol/test`        |   169 | The wire protocol, login and RSA, result sets, data types, SQL construction, error mapping, socket lifecycle                                                                                          |
| `export/test`        |   151 | CSV, XLSX, Parquet and Thrift encoding; SVG, PNG and PDF figures; streaming row walks; sinks                                                                                                          |
| `worker/test`        |    89 | The data worker and its client, the table controller, endpoints, export streaming, and the latency invariance test                                                                                    |
| `renderer/test`      |   510 | Draw lists for tables, charts, query boxes, summary panels; the halo; connector routing; hit testing; interaction; glyph atlas and text; camera, LOD, quad batching, frame stats, XR                  |
| `mcp/test`           |   229 | The protocol — answered by the server and answered in the page — the tool catalogue and its handlers, command parsing, snapshots, the HTTP route, the bridge, the stdio host, Claude discovery        |
| `ui/test`            |    89 | The React panels, through their accessible roles                                                                                                                                                      |
| `apps/web/test`      |   460 | The composition root: the workspace and its collaborators, the canvas, the editors, the agent bridge and the desktop shell's, startup, placement, export jobs, the install shell and its cache policy |
...
| `docs/AGENT-SKILL-CHARTS.md` ↔ the ECharts seam                                | The chart page's claims still hold                          | Written with the page: which series draw, which are inert, and that a named CSS colour makes a mark vanish are facts about this integration, invisible in the library's own docs, and would not survive a version bump unwatched |
| `@panorama/json-tables` ↔ the two loaders' own sources                         | The column grammar still matches what they emit             | Written with the parser: it models somebody else's contract, every case in its tests is one those projects actually produce, and a shape nobody produces is a case not worth being right about                                   |
...
npm run dev:probe                 # in one terminal, serves on 5199
...
**`npm run shell-check`** is the newest and exists because of a specific failure.
Everything the desktop shell answers reaches the page over an IPC call a browser
does not have, so those parts of the interface are the one place a unit test can
pass while the application is broken — and one did. The local-deployments list was
written, unit-tested, built and shipped _not working_: the sidebar is a `useMemo`
whose dependency list did not name the state the list is drawn from, so React kept
serving the memoised element and the panel stayed empty. `App.test.tsx` passed
because jsdom re-rendered the sidebar for an unrelated reason. This probe stubs the
shell's global in a real browser, drives the built page, and asserts what is on the
screen: the deployment rows, that a stopped one is inert, that clicking a running
one asks for its credentials, that the settings panel shows the endpoint and the
Claude the shell found — and that a page with no shell is offered none of it.

One more script belongs beside these and is not a browser probe:
**`npm run desktop-check`** drives the _built desktop application_ over its own
agent endpoint and its own stdio pipe — no Playwright, no page — and asserts the
six claims that make an installed Panorama self-contained. They are listed at the
top of `scripts/desktop-check.mjs`; the shortest of them is that a call with
nothing running opens a window and is answered. It keeps its session file in a
directory of its own, so a run does not disturb an application you have open.
...
| `npm run sql-check`      | The query box: greying on synthetic relations, drag, composition, highlighting                                                                              | Half of it is canvas and half is a DOM overlay sitting exactly on top                         |
...
| `npm run update-check`   | An installed application updating itself: build one version, stage another, quit, and assert the second one came back                                       | Every other test covers a part of the updater; only this one covers the mechanism             |
| `npm run resize-check`   | A window resized four times at 2x: whether the drawing buffer is reallocated, and whether the picture moves, stretches, uncovers or loses its device pixels | Four different faults look identical to whoever is dragging the corner                        |
| `npm run agent-check`    | The endpoint, the tools, an edit, the refusals, a chart nobody is looking at, the stdio pipe, and a client told its tool list went stale                    | Three parts in three processes that can only be wrong together                                |
...
`export-check`, `resize-check`, `update-check` and `install-check` set a non-zero
exit code. The
others report and leave the judgement to the reader; see §12.
...
| Gap                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Consequence                                                                                                                                                                                                                                                                                                                                             |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **The probes report; they do not assert.** Only `export-check`, `resize-check`, `update-check` and `install-check` set an exit code. The rest print a JSON report for a human or an agent to read.                                                                                                                                                                                                                                                                                                                                                                                                     | A probe can regress into nonsense and still "pass". Each should assert its own report and exit non-zero.                                                                                                                                                                                                                                                |
| **CI runs the gate, not the probes.** `.github/workflows/verify.yml` runs `format:check`, `npm run verify` and `install-check` on every push and pull request; `release.yml` builds the installable application, drives that exact build in a browser and publishes it. The other ten probes are still run by hand, because they report rather than assert (above).                                                                                                                                                                                                                                    | A regression the suite or the installed app can show is caught now; one only a report-only probe would have shown is not.                                                                                                                                                                                                                               |
...
| **Two things ask the machine, so two tests are `#[ignore]`d.** Finding Claude Code, and listing Exasol Personal's deployments and reading one's password — the latter now also asserting the invariant that caught a real bug: a deployment is offered as connectable _if and only if_ `exasol status` called it `database_ready`. Both are covered structurally — parsing what the tools print, the address a deployment reports, where a password lives — and the ignored pair runs the real commands. They were last run with `PATH=/usr/bin:/bin`, which is what a Dock-launched application gets. | On a machine without those tools the ignored tests cannot run at all, so a change to how they are invoked is only caught by somebody running `cargo test -- --ignored`. The click that turns a listed deployment into a connection is covered in `ConnectionDialog.test.tsx` and `App.test.tsx`; against a real database it has only been done by hand. |
| **Pairing touches the machine, so the suite cannot.** `claude.rs` runs Claude Code's own command and writes the desktop client's configuration file; what is tested is the merge (a file that is somebody else's, one that is not JSON, an `mcpServers` that is not an object) and that the pairing names this executable rather than a port. Finding Claude at all has an `#[ignore]`d test, because it needs a machine with Claude installed.                                                                                                                                                        | The path that fails silently is a Dock-launched application inheriting almost no `PATH`; the search asks the login shell for that reason, and only the ignored test exercises it. Run `cargo test -- --ignored` after touching it.                                                                                                                      |
| **Nothing has been run on Windows or Linux.** The desktop shell is written for three platforms and has been built and driven on one. The release workflow builds all three and runs `cargo test` on each, so the platform-specific code — finding a `.cmd` shim rather than an executable, running one through the command interpreter, the stubs where a unix call has no equivalent — is compiled and unit-tested there; nothing has _used_ the application on either.                                                                                                                               | A Windows-only defect reaches a release unless somebody runs it. The lookup and execution rules that differ per platform are unit-tested and were written from the documented behaviour of `CreateProcess`, not from a run.                                                                                                                             |
| **The certificate dialog needs a human.** The desktop application asks about an unverifiable certificate in a native dialog, and nothing automated can click it. The decision either side of it is covered (`cargo test` for loopback, remembering, and the wording; the probe for the socket's own guards), and both the loopback and the remembered paths were driven against a real self-signed instance by hand.                                                                                                                                                                                   | The one path nobody has exercised end to end is a person pressing "Trust and connect". A refusal after that dialog is likewise only structurally tested.                                                                                                                                                                                                |
| **`shell-check` is not in CI either.** It needs Chromium and a build, which CI has for `install-check` — so this is a gap of omission rather than of possibility.                                                                                                                                                                                                                                                                                                                                                                                                                                      | Until it runs there, the class of bug it was written for (shell-fed UI that renders nothing) is caught only by somebody running it.                                                                                                                                                                                                                     |
| **The desktop probe is not in CI.** `npm run desktop-check` drives the built application the way an agent would — endpoint, catalogue, an edit that lands, the rows behind it, the guards, the pipe answering with no window open and starting one when a call needs it — and `cargo test` covers the shell's own decisions. Neither the probe nor a bundled window has ever run on a hosted runner.                                                                                                                                                                                                   | A webview-only or packaging-only regression reaches a release unless somebody runs the probe on a build. The Rust tests do run in CI (`verify.yml`), so the half that needs no screen is covered.                                                                                                                                                       |
